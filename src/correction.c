...
int encodeParity(int n, int p, int data){
    ...
    for(int i=1;i<n+p+1;i++){
        if(checkPowerOfTwo(i)) result |= 0;
        else result |= getBit(data, n - dataIndex++ - 1); //첫번째 부터
        if(i!=(n+p)) result = result << 1;
    }
