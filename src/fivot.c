...
int ** new_fivot_bitmap(int chunk_size){
...
void set_fivot_bitmap(int ** bitmap, int chunk_size, BITFILE * bf){
    for(int i=0;i<chunk_size;i++){
        for(int j=0;j<8;j++){
            bread(INT, bf, &bitmap[i][j], 1, 1);
        }
...
void print_fivot_bitmap(int ** bitmap, int chunk_size){
    for(int i=0;i<chunk_size;i++){
        for(int j=0;j<8;j++){
            printf("%d ", bitmap[i][j]);
        }
...
void free_fivot_bitmap(int ** bitmap, int chunk_size){
    for(int i=0;i<chunk_size;i++){
        free(bitmap[i]);
    }
    free(bitmap);
...
void revert_fivot(char * input, char * output){
    int chunk = filesize(input);
    int ** bitmap = new_fivot_bitmap(chunk);
